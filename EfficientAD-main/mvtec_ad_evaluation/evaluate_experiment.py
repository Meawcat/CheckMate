...
def parse_dataset_files(object_name, dataset_base_dir, anomaly_maps_dir):
    ...
    test_dir = path.join(dataset_base_dir.replace('/', '\\'), object_name, 'test')
    gt_base_dir = path.join(dataset_base_dir.replace('/', '\\'), object_name, 'ground_truth')
    anomaly_maps_base_dir = path.join(anomaly_maps_dir.replace('/', '\\'), object_name, 'test')
...
def calculate_au_pro_au_roc(gt_filenames,
    ...
    for (gt_name, pred_name) in tqdm(zip(gt_filenames, prediction_filenames),
        ...
        prediction = util.read_tiff(pred_name, exts='tiff')
